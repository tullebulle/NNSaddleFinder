...
train_dir_path = 'nequip_data/toluene/example-run-toluene'
...
test_data = np.load('nequip_data/benchmark_data/toluene_ccsd_t-test.npz', allow_pickle=True)
test_idx = 0
...
def nequip_potential(flat_positions):
...
def grad_potential(flat_positions):
    positions = flat_positions.reshape(-1, 3)
    input_dict[AtomicDataDict.POSITIONS_KEY] = positions #might need to require grad
    out = model(input_dict)
