...
from function_wrapper_aspirin import nequip_potential_aspirin, flat_positions as initial_flat_positions
import torch
import psutil
import os
...
NORM_HISTORY_FILE = os.path.join(os.path.dirname(SAVE_FILE), "norm_history_aspirin.pt")
...
evals, evecs = torch.linalg.eigh(torch.autograd.functional.hessian(nequip_potential_aspirin, flat_positions))
...
if optimize:
    ...
    for step in range(n_steps):
        ...
        if not flat_positions.requires_grad:
            flat_positions.requires_grad_(True)
        out = nequip_potential_aspirin(flat_positions)
    ...
    out_final = nequip_potential_aspirin(flat_positions)
    ...
    if best_positions is not None:
        ...
        out_best = nequip_potential_aspirin(best_positions)
